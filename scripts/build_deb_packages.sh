...
EOF
    ...
    case "$arch" in
        "amd64")
            ...
            if [ -d src/nymyac ]; then
                cd src/nymyac && cargo build --target $TARGET --release && cd ../..
                if [ -f "src/nymyac/target/$TARGET/release/nymyac" ]; then
                    cp "src/nymyac/target/$TARGET/release/nymyac" "${pkg_dir}/usr/bin/nymyac"
                else
                    echo "Error: Binary not found at src/nymyac/target/$TARGET/release/nymyac"
                    exit 1
                fi
            elif [ -d nymyac ]; then
                ...
                echo "Error: nymyac directory not found in either src/nymyac or root"
                exit 1
            fi
            ;;
        "arm64")
    ...
    if [ -d "src/nymyac/library" ]; then
        LIB_DIR="src/nymyac/library"
    elif [ -d "nymyac/library" ]; then
        LIB_DIR="nymyac/library"
    else
        echo "Error: library directory not found in either src/nymyac/library or nymyac/library"
        exit 1
    fi
    ...
    for subdir in math math/gmp math/hypercalc math/functions ml ml/classical ml/quantum_ml networking networking/classical networking/quantum quantum quantum/gate quantum/sim quantum/alg datetime crystal crystal/io lowlevel physics physics/quantum system; do
        if [ -d "${LIB_DIR}/$subdir" ]; then
            ...
            find "${LIB_DIR}/$subdir" -name "*.nym" -type f -exec cp {} "${pkg_dir}/usr/lib/nymya/$subdir/" \; -exec echo "  Copied: {}" \;
        fi
    done
...
EOF
    ...
    if [ -d "src/syntax_highlighter" ]; then
        mkdir -p "${pkg_dir}/usr/share/nymya/syntax_highlighter"
        cp -r src/syntax_highlighter/* "${pkg_dir}/usr/share/nymya/syntax_highlighter/"
        ...
        if [ -f "src/syntax_highlighter/nym.vim" ]; then
            cp "src/syntax_highlighter/nym.vim" "${pkg_dir}/usr/share/vim/vimfiles/syntax/"
        fi

        # Copy to Nano syntax directory if available
        if [ -f "src/syntax_highlighter/nym.nanorc" ]; then
            cp "src/syntax_highlighter/nym.nanorc" "${pkg_dir}/usr/share/nano/"
        fi

        # Copy to GTKSourceView language specs if available
        if [ -f "src/syntax_highlighter/nymya.lang" ]; then
            cp "src/syntax_highlighter/nymya.lang" "${pkg_dir}/usr/share/gtksourceview-4/language-specs/"
        fi

        # Copy to highlight language definitions if available
        if [ -f "src/syntax_highlighter/nymya.xml" ]; then
            cp "src/syntax_highlighter/nymya.xml" "${pkg_dir}/usr/share/highlight/langDefs/"
        fi

        echo "  Copied: syntax highlighting files"
    elif [ -d "syntax_highlighter" ]; then
        mkdir -p "${pkg_dir}/usr/share/nymya/syntax_highlighter"
