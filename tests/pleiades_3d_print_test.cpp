 ...
 * Auto-generated from tests/pleiades_3d_print_test.nym
 */
...
int main() {
    try {
        ...
        // crystal::manifest("Compiled from: tests/pleiades_3d_print_test.nym");
    ...
    self->luminosity_sol();
    self->position();
    graphics::ply_basic::spherical_to_cartesian(ra, dec, dist);
    ...
    auto scale_factor = math::pow(star, ., luminosity_sol, 1.0, /, 3.0);
    crystal::manifest("Scale factor: ", +, graphics, ., stl_basic, ., float_to_string, (, scale_factor);
    auto bar_radius = 0.2;
    stars.size();
    auto current_star = stars[i];
    auto nearest_idx = -;
    stars.size();
